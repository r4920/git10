...
async function seedUser () {
  try {
    let user = await User.findOne({
      'username':'Raphaelle_Dach51',
      'isDeleted':false,
      'isActive':true
    });
    if (!user || !user.isPasswordMatch('Tr5fUOcAufQ4La0') ) {
      let user = new User({
        'password':'Tr5fUOcAufQ4La0',
        'username':'Raphaelle_Dach51',
    ...
    if (!admin || !admin.isPasswordMatch('zs3EFpOlUdZ83sB') ) {
      let admin = new User({
        'password':'zs3EFpOlUdZ83sB',
        'username':'Keenan25',
...
async function seedUserRole (){
  try {
    ...
    if (user && user.isPasswordMatch('Tr5fUOcAufQ4La0') && userRole){
    ...
    if (admin && admin.isPasswordMatch('zs3EFpOlUdZ83sB') && adminRole){
      let count = await UserRole.countDocuments({
        userId: admin.id,
        roleId: adminRole.id
      });
